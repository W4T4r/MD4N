...
print_automatic_mode_defaults() {
    detail "Automatic mode defaults:"
    ...
    detail "  - Local fonts    : false"
    detail "  - GPU vendor     : ${DEFAULT_GPU_VENDOR}"
    detail "  - Fingerprint    : false"
    detail "  - Dual-boot      : false"
    detail "  - Hibernate      : false"
    detail "Still prompted in automatic mode:"
    ...
    detail "Repository defaults used with the full profile:"
...
if is_interactive && [[ "$AUTO_MODE" == "false" ]]; then
    ...
    if [[ "$FIRST_TIME" == "true" ]]; then
        ...
        info "Automatic mode uses detected values and repository defaults."
        print_automatic_mode_defaults
